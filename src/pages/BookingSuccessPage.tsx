    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-12">
            ...
            <div className="text-center mb-8">
                <div className={`w-20 h-20 mx-auto rounded-full flex items-center justify-center mb-6 ${isConfirmed ? 'bg-green-100' : 'bg-yellow-100'
                    }`}>
                    {isConfirmed ? (
            ...
            <div className="bg-white border border-secondary-200 rounded-xl overflow-hidden mb-8">
                ...
                <div className="flex gap-4 p-6 border-b border-secondary-200">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-sm text-secondary-500 capitalize">{property.propertyType.replace('_', ' ')}</p>
                        <h2 className="font-semibold text-lg truncate">{property.title}</h2>
                ...
                <div className="p-6 space-y-4">
                    <div className="flex justify-between items-center py-3 border-b border-secondary-100">
                        <div>
                            <p className="text-sm text-secondary-500">Check-in</p>
                            <p className="font-medium">{format(booking.checkIn.toDate(), 'EEE, MMM d, yyyy')}</p>
                            <p className="text-sm text-secondary-500">{property.checkInTime}</p>
                        </div>
                        <svg className="w-6 h-6 text-secondary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                        </svg>
                        <div className="text-right">
                            <p className="text-sm text-secondary-500">Check-out</p>
                            <p className="font-medium">{format(booking.checkOut.toDate(), 'EEE, MMM d, yyyy')}</p>
                            <p className="text-sm text-secondary-500">{property.checkOutTime}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between py-3 border-b border-secondary-100">
                        <div>
                            <p className="text-sm text-secondary-500">Confirmation Code</p>
                    ...
                    <div className="flex justify-between pt-3">
                        ...
                        <p className="font-semibold text-lg">{formatPrice(booking.pricing.total)}</p>
                    </div>
                </div>
            </div>
